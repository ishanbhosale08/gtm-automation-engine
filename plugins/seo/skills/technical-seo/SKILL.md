...
- **GTM Automation Engine Audit Checklist** – five-pass review (crawl → index → performance → schema → reporting) README.md#183-212.
...
- KPI guardrail sheet (sessions, rankings, conversions) with "alert" thresholds matching GTM Automation Engine governance cadence.
...
- Share Guardrail snapshots weekly using GTM Automation Engine-style status packet (target vs actual vs guardrail) to keep Sales + Execs aligned.
- Trigger rollback when three guardrail breaches occur in 24h or when crawl errors spike >25% vs baseline.

## GTM Automation Engine SEO Escalation Workflow
